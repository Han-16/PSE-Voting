...
use ark_bn254::{
    Bn254, Fq as Basefield, Fr as Scalarfield, G1Affine as GAffine, G1Projective as GPro,
    G1Projective as GPro2, G2Affine as GAffine2,
};
...
use std::ops::{Add, Mul}; // Mul 트레이트 임포트
...
impl KZGsetup for CRS {
    fn for_s1(x: &Basefield) -> Vec<Basefield> {
        //x >> [1,0, ...]
    ...
    fn vanishing_polynomial(scol: &IR) -> DensePolynomial<Basefield> {
        ...
        for k in 1..scol.Witness[0].len() + 1 {
            let k_value = Basefield::from(k as u64); // k 값을 Basefield로 변환
                                                     // (x - k) 다항식 생성
            let x_minus_k =
                DensePolynomial::from_coefficients_vec(vec![-k_value, Basefield::one()]);
...
pub trait IRtoPoly {
    ...
    fn add_polynomials(
        poly1: &DensePolynomial<Basefield>,
        poly2: &DensePolynomial<Basefield>,
    ) -> DensePolynomial<Basefield>;

    fn lagrange_interpolation_polynomial(
        evaluation_values: &Vec<Basefield>,
    ) -> DensePolynomial<Basefield>;
...
impl IRtoPoly for IR {
    ...
    fn lagrange_basis_polynomial(i: usize, num_points: usize) -> DensePolynomial<Basefield> {
        ...
        for j in 0..num_points {
            if i != j {
                ...
                let temp_numer =
                    DensePolynomial::from_coefficients_vec(vec![-xj, Basefield::one()]);
    ...
    fn lagrange_interpolation_polynomial(
        ...
        for (i, &yi) in evaluation_values.iter().enumerate() {
            ...
            // 직접 다항식 곱셈 구현
            interpolated_poly = IR::add_polynomials(
                &interpolated_poly,
                &IR::multiply_polynomials(&basis_poly, &yi_poly),
            );
        }
    ...
    fn add_polynomials(
        ...
        let mut result_coeffs =
            vec![Basefield::zero(); std::cmp::max(coeffs1.len(), coeffs2.len())];
    ...
    fn divide_polynomials(
        ...
        let divisor_lead_inv = divisor.coeffs()[divisor_degree]
            .inverse()
            .expect("Divisor's leading coefficient should have an inverse");
...
impl proofgen for Prover {
    ...
    fn for_scalarmul_X2Y2(sk: Basefield, point: GPro) -> Vec<GAffine> {
        ...
        for k in sk_bits {
            if k == true {
                //1이라면 [sum ri]g의 Affine을 넣음
                points.push(point.into_affine());
            } else if k == false {
                //0이라면 아무것도 넣지 않음, 아무것도 넣지 않는 대신에 dummy 채움
    ...
    fn for_scalarmul_X1Y1Z1(sk: Basefield, point: GPro) -> Vec<GPro> {
        ...
        for k in sk_bits {
            if k == true {
                //1이라면 [sum ri]g를 이전 결과에 더한 게 입력되어야함
                res = res + point;
...
fn main() {
    ...
    let mut execution_trace = IR {
        Witness: Vec::new(),
    };
    execution_trace
        .Witness
        .push(IR::for_interpol_ProjX(&X1Y1Z1));
    execution_trace
        .Witness
        .push(IR::for_interpol_ProjY(&X1Y1Z1));
    execution_trace
        .Witness
        .push(IR::for_interpol_ProjZ(&X1Y1Z1));

    execution_trace
        .Witness
        .push(IR::for_interpol_AffineX(&X2Y2));
    execution_trace
        .Witness
        .push(IR::for_interpol_AffineY(&X2Y2));
    ...
    // - mixed addition
    let m_S1 = IR::multiply_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::from(-1)]),
        &pv.crs.circuit[0],
    ); //-S1(X)
    let m_S2 = IR::multiply_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::from(-1)]),
        &pv.crs.circuit[1],
    ); //-S2(X)
    let one_m_S1 = IR::add_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::one()]),
        &m_S1,
    ); //(1-S1(X))
    let one_m_S2 = IR::add_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::one()]),
        &m_S2,
    ); //(1-S2(X))
    ...
    let mut mix_tmp1 = IR::multiply_polynomials(&pv.witness_poly[1], &pv.witness_poly[4])
        + IR::multiply_polynomials(
            &DensePolynomial::from_coefficients_vec(vec![Basefield::from(-9)]),
            &pv.witness_poly[2],
        );
    let mut mix_tmp2 = IR::add_polynomials(
        &pv.witness_poly[1],
        &IR::multiply_polynomials(&pv.witness_poly[4], &pv.witness_poly[3]),
    );
    let mut mix_tmp3 = IR::add_polynomials(
        &pv.witness_poly[0],
        &IR::multiply_polynomials(&pv.witness_poly[3], &pv.witness_poly[2]),
    );
    ...
    mix_add2 = mix_add2
        + IR::multiply_polynomials(
            &DensePolynomial::from_coefficients_vec(vec![Basefield::from(27)]),
            &mix_tmp4,
        );
    ...
    let mut mix_add3 = IR::add_polynomials(
        &pv.witness_poly[1],
    ...
    mix_tmp2 = IR::multiply_polynomials(&pv.witness_poly[0], &pv.witness_poly[3]);
    mix_tmp3 = IR::add_polynomials(
        &IR::multiply_polynomials(&pv.witness_poly[0], &pv.witness_poly[4]),
        &IR::multiply_polynomials(&pv.witness_poly[3], &pv.witness_poly[1]),
    );
    let mut mix_tmp4 = IR::multiply_polynomials(&mix_tmp2, &mix_tmp3);
    ...
    mix_add3 = mix_add3
        + IR::multiply_polynomials(
            &DensePolynomial::from_coefficients_vec(vec![Basefield::from(3)]),
            &mix_tmp4,
        );
    ...
    //-doubling
    let mut doub1 = IR::multiply_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::from(2)]),
    ...
    let mut doub_tmp2 = IR::multiply_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::from(-27)]),
        &doub_tmp1,
    );
    ...
    doub_tmp2 = IR::multiply_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::from(-27)]),
        &doub_tmp1,
    );
    doub_tmp4 = IR::multiply_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::from(9)]),
        &doub_tmp1,
    );
    ...
    doub_tmp4 = IR::multiply_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::from(24)]),
        &doub_tmp1,
    );
    ...
    let mut doub3 = IR::multiply_polynomials(
        &DensePolynomial::from_coefficients_vec(vec![Basefield::from(8)]),
        &doub_tmp3,
    );
    ...
    //-Identity addition
    let mut iden1 = IR::add_polynomials(
        &pv.witness_poly[0],
        &IR::multiply_polynomials(
            &DensePolynomial::from_coefficients_vec(vec![Basefield::from(-1)]),
            &pv.witness_poly[5],
    ...
    let mut iden2 = IR::add_polynomials(
        &pv.witness_poly[1],
        &IR::multiply_polynomials(
            &DensePolynomial::from_coefficients_vec(vec![Basefield::from(-1)]),
            &pv.witness_poly[6],
    ...
    let mut iden3 = IR::add_polynomials(
        &pv.witness_poly[2],
        &IR::multiply_polynomials(
            &DensePolynomial::from_coefficients_vec(vec![Basefield::from(-1)]),
            &pv.witness_poly[7],
    ...
    total_circuit = total_circuit
        + IR::multiply_polynomials(
            &IR::multiply_polynomials(&pv.crs.circuit[0], &one_m_S2),
            &doubling,
        );
    total_circuit = total_circuit
        + IR::multiply_polynomials(
            &IR::multiply_polynomials(&one_m_S1, &pv.crs.circuit[1]),
            &identy_check,
        );
    total_circuit = total_circuit
        + IR::multiply_polynomials(
            &IR::multiply_polynomials(&one_m_S1, &one_m_S2),
            &equality_check,
        );
