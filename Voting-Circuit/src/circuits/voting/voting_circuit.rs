...
use ark_crypto_primitives::{crh::{poseidon::constraints::{CRHGadget, CRHParametersVar, TwoToOneCRHGadget}, CRHSchemeGadget, TwoToOneCRHSchemeGadget}, sponge::{poseidon::PoseidonConfig, Absorb}};
    ...
    for<'a> &'a GG: GroupOpsBounds<'a, C, GG>,
{
    pub g: C::Affine,
    pub ck: Vec<C::Affine>,
    pub params: PoseidonConfig<C::BaseField>,
    ...
    for<'a> &'a GG: GroupOpsBounds<'a, C, GG>,
{
    fn generate_constraints(self, cs: ConstraintSystemRef<C::BaseField>) -> ark_relations::r1cs::Result<()> {
        ...
        let params = CRHParametersVar::<C::BaseField>::new_constant(cs.clone(), self.params)?;
        ...
        let addr_computed = CRHGadget::<C::BaseField>::evaluate(&params, &hash_input)?;
        addr_computed.enforce_equal(&addr)?;


        // 3. Check sn = CRH(voting_round || sk)
        let hash_input = vec![voting_round, sk];
        let sn_computed = CRHGadget::<C::BaseField>::evaluate(&params, &hash_input)?;
